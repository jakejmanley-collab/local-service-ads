...
function DashboardContent() {
  ...
  const [hasPremiumFlyers, setHasPremiumFlyers] = useState(false);
  ...
  useEffect(() => {
    ...
    if (assets) setSavedAssets(JSON.parse(assets));
    ...
    // 3. Handle Upgrades via URL parameters (Simulating a webhook for MVP)
    const upgrade = searchParams.get('upgrade');
    if (upgrade === 'verified' || upgrade === 'proplus') {
      localStorage.setItem('aretifi_plan', upgrade);
    } else if (upgrade === 'flyer') {
      localStorage.setItem('aretifi_flyers', 'premium');
    }

    // 4. Set state based on saved upgrades
    const currentPlan = localStorage.getItem('aretifi_plan');
    if (currentPlan === 'verified' || currentPlan === 'proplus') setIsVerified(true);
    if (localStorage.getItem('aretifi_flyers') === 'premium') setHasPremiumFlyers(true);
  ...
  const handleDownload = async (elementId: string, shape: string) => {
    ...
    try {
      ...
      link.download = `${businessName.replace(/\s+/g, '-')}-${shape}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {!hasPremiumFlyers ? (
            ...
            <div className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-2xl p-6 text-white shadow-lg">
              ...
              <p className="text-sm text-blue-100 font-medium leading-relaxed">You have unlocked premium business flyers. High-resolution downloads are now enabled.</p>
            </div>
          )}

          {/* Conditional Plan Pitch */}
          {!isVerified ? (
            ...
            <div className="bg-white border border-green-200 rounded-2xl p-6 shadow-sm flex flex-col items-center text-center">
              <div className="w-12 h-12 bg-green-50 rounded-full flex items-center justify-center mb-4">
                <svg className="w-6 h-6 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
              </div>
              <h3 className="font-bold text-lg text-slate-900 mb-1">Verified Pro Active</h3>
              <p className="text-sm text-slate-500 font-medium">Your profile is now verified and your professional domain is active.</p>
            </div>
          )}
        </div>
      </div>
...
export default function DashboardPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f8fafc] flex items-center justify-center text-slate-400 font-bold">Loading Dashboard...</div>}>
      <DashboardContent />
    </Suspense>
  );
}
